...
class Grid {
    ...
    int distanceToCenter(int x,
                         int y) {
        return abs(x - 5) + abs(y - 5);
    }

    void replaceAround(int x,
                       int y,
                       char from,
                       char to) {
        int[] around = new int[]{-1, 0, 1};
    ...
    public List<TestCase<Integer>> generate() {
        ...
        TestCase<Integer> test = new TestCase<Integer>()
                ...
                .addInput(out -> {
                    ...
                    char checkStar = grid.get(state.xStar,
                                              state.yStar);
                    if (checkStar != '*') {
                        return CheckResult.wrong(
                              ...
                              .addInput(5,
                                        out -> {
                                            ...
                                            for (int x = 1; x <= 9; x++) {
                                                for (int y = 1; y <= 9; y++) {
                                                    int numsAround = 0;
                                                    if (state2.first.get(x,
                                                                         y) == '.') {
                                                        for (char c = '1'; c <= '9'; c++) {
                                                            numsAround += state2.first.countAround(x,
                                                                                                   y,
                                                                                                   c);
                                                        }
                                                    }
                                                    ...
                                                    if (numsAround == maxNumsAround) {
                                                        int currDistToCenter =
                                                                state2.first.distanceToCenter(x,
                                                                                              y);
